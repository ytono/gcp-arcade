...
CVE-2025-0426: Kubelet Checkpoint APIを介したノードのサービス拒否 (DoS)

何が問題か？ 認証されていないKubeletの読み取り専用HTTPエンドポイントに対して、大量のコンテナチェックポイントリクエストを送信されると、ノードのディスク容量が一杯になり、ノードが動作しなくなる可能性があります（サービス拒否）。
影響を受けるバージョン： Kubeletのバージョンが以下のものが該当します。
v1.30.0 ～ v1.30.9
v1.31.0 ～ v1.31.5
v1.32.0 ～ v1.32.1
修正されたバージョン： 以下のバージョンにアップデートしてください。
1.29.14
1.30.10
1.31.6
1.32.2
誰が報告・修正したか？ GoogleのTim Allclair氏 (@tallclair) が報告し、修正しました。
重要度： Medium (中程度)
攻撃はローカルから可能
特別な権限は不要
ユーザーの操作は不要
機密性・完全性への影響はなし
可用性（システムが利用できる状態）への影響は大
変更点

機能

Kubernetesのビルドに使用するGo言語のバージョンが1.22.11から1.22.12になりました。
バグ修正

CRD (Custom Resource Definition) に必要なバージョン情報がない場合に、kube-apiserverのOpenAPIコントローラーで使用されるBuildOpenAPIV2およびBuildOpenAPIV3ユーティリティで発生するnil pointer panic (プログラムが誤ったメモリ領域にアクセスしようとして停止するエラー) を修正しました。
Portworxのセキュリティ機能が有効になっているクラスタで、PortworxボリュームのインツリーからCSIへの移行を修正しました。これにより、インツリーStorageClassで指定されたSecretのデータがCSIリクエストに正しく渡されるようになります。
kubeadm: CRIからのサンドボックスイメージにエラーがある場合に、イメージがプルされないバグを修正しました。
kubeadm: v1beta4のTimeouts.EtcdAPICallフィールドがetcdクライアントの操作で無視され、常にデフォルトの2分間のタイムアウトが使用されていたバグを修正しました。
その他
