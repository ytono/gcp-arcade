Kubernetes v1.31.8 リリースノート
このバージョンは、主に機能更新とバグ修正を含みます。

1. Go 1.23.8でのビルド

変更内容 (Feature): KubernetesがGo 1.23.8でビルドされるようになりました。 (Go 1.23.8でKubernetesビルド #131537)
プラットフォームへの影響: 中
理由: Kubernetesのコンポーネントが新しいGo言語バージョンでビルドされることで、パフォーマンスの向上、メモリ使用量の削減、セキュリティ修正など、基盤レベルでの改善が期待されます。Goランタイムの変更により、まれに微細な挙動の変化や、特定のGoバージョンに依存するツールとの互換性問題が発生する可能性はゼロではありませんが、通常は安定性向上が目的です。
対処方法:
アップグレード前に、テスト環境でアプリケーションとの互換性、安定性、リソース消費量（CPU/メモリ）の変化に注意して十分な動作確認を行ってください。
もしクラスタ内でカスタムビルドされたコンポーネントや、特定のGoバージョンに強く依存するツールを使用している場合は、それらのコンポーネントが新しいGoバージョンと互換性があることを確認してください。
2. kubeletのリカバリ機能におけるリサイズのチェック改善

変更内容 (Bug or Regression): kubeletがリカバリ機能のステータスを決定する際に、より新しいリサイズフィールドをチェックするように修正されました。(kubeletでのリカバリ機能ステータス決定時の新しいリサイズフィールドチェック #131439)
プラットフォームへの影響: 低〜中
理由: これはkubeletのバグ修正で、ストレージのリカバリ関連のロジックが改善されます。特に、PersistentVolumeClaim (PVC) のリサイズに絡むシナリオでの安定性が向上することが期待されます。既存のリサイズ失敗ケースを抱えている環境であれば、この修正によって問題が解消される可能性があります。
対処方法:
特に対処は不要ですが、PVCのリサイズ機能を頻繁に利用している環境であれば、この修正により安定性が向上するため、アップグレードを検討する価値があります。
3. Windows ProxyでのHNSエンドポイント作成失敗の解消

変更内容 (Bug or Regression): バージョン1.31で導入されたWindows Proxyの退行バグを解消しました。このバグは、同じIPアドレスを持つリモートHNSエンドポイントが既に作成されている場合、HNSエンドポイントの作成が失敗する問題を修正します。(Windows ProxyでのHNSエンドポイント作成失敗の解消 #131429)
プラットフォームへの影響: 高 (Windowsノードを使用している場合)
理由: KubernetesクラスタでWindowsノードを使用しており、特にkube-proxyがWindows Proxyモードで動作している場合に影響する重要なバグ修正です。同じIPアドレスを持つHNS (Host Network Service) エンドポイントが存在すると、新規のPodやサービスに対するHNSエンドポイントの作成が失敗し、ネットワーク接続性が確立できない、またはサービスルーティングに問題が生じる可能性があります。これはWindowsノードにおけるPodのデプロイや通信に深刻な障害をもたらします。
対処方法:
Windowsノードを使用しているKubernetesクラスタは、速やかにこのバージョンにアップグレードすることを強く推奨します。
アップグレード前に、既存のWindowsノードのHNSエンドポイントの状態を確認し、潜在的なIPアドレスの競合がないか検証してください。
アップグレード後には、Windows上のPodのネットワーク接続性、特にServiceへのアクセスが正常であることを確認してください。
Kubernetes v1.31.7 リリースノート
このバージョンは、主に深刻なバグ修正を含みます。

1. kube-apiserverのWatchイベントに関するバグ修正

変更内容 (Bug or Regression): kube-apiserverが、以前のイベントの復号化に失敗し、それが発行されなかった場合でも、さらなるwatchイベントを発行してしまうバグを修正しました。(kube-apiserverでのwatchイベント誤発行バグ修正 #131160)
プラットフォームへの影響: 中〜高
理由: kube-apiserverのwatchメカニズムは、Kubernetesのコントローラー（kube-controller-manager、各種オペレーター、カスタムコントローラーなど）がクラスタの状態変化をリアルタイムで取得するために非常に重要です。このバグにより、不正なwatchイベントの発行や、イベントの欠落が発生し、コントローラーが古い情報に基づいて動作したり、特定のAPIクライアントが不安定になったりする可能性があります。これにより、クラスタ全体の調整機能や自動化機能の信頼性が低下する可能性があります。
対処方法:
カスタムコントローラーやオペレーターなど、watchメカニズムに強く依存するコンポーネントを使用している環境では、この修正によって安定性が向上する可能性があります。
アップグレード前に、apiserverのログやメトリクスを監視し、異常がないか確認してください。
アップグレード後も、各コントローラーが正常に動作しているか、APIからのイベント処理に問題がないかを継続的に監視してください。
2. kubelet再起動時のPodボリュームのアンマウント問題修正

変更内容 (Bug or Regression): ユーザーによって参照されているPVCが削除されている場合、kubeletの再起動時に実行中のPodのボリュームがアンマウントされてしまうバグを修正しました。(kubelet再起動時のPodボリューム不適切アンマウント修正 #130685)
プラットフォームへの影響: 高
理由: 実行中のPodが利用しているPersistentVolumeClaim (PVC) がユーザーによって削除プロセス中（削除が完了していない状態など）にある状況でkubeletが再起動すると、そのPodのボリュームが予期せずアンマウントされてしまうという、非常に深刻なバグです。これにより、Podがデータにアクセスできなくなり、アプリケーションの停止や、最悪の場合、サービスやデータ破損につながる可能性があります。特にデータベースやメッセージキューなど、ストレージに永続的に依存するステートフルアプリケーションにとって大きなリスクです。
対処方法:
ステートフルなアプリケーションやPVCを頻繁に利用している環境では、速やかにこのバージョン（またはこの修正が適用されたより新しいパッチバージョン）にアップグレードすることを強く推奨します。
アップグレード前に、実行中のPodが使用しているPVCを削除する操作（特にノードの再起動が予定されている場合）を避けるように運用フローを見直してください。
アップグレード後、ストレージの安定性とPodのボリュームマウント状態を継続的に監視してください。
Kubernetes v1.32.3 リリースノート
このバージョンは、主にバグ修正を含みます。v1.31.7のいくつかの修正がv1.32系にもバックポートされています。

1. kube-apiserverのWatchイベントに関するバグ修正

変更内容 (Bug or Regression): kube-apiserverが、以前のイベントの復号化に失敗し、それが発行されなかった場合でも、さらなるwatchイベントを発行してしまうバグを修正しました。(kube-apiserverでのwatchイベント誤発行バグ修正 #131159)
プラットフォームへの影響: 中〜高
理由: これはv1.31.7で修正されたものと同様の、kube-apiserverのwatchメカニズムに関するバグです。v1.32系列のクラスタの場合も、APIの安定性とイベントの一貫性に影響を与える可能性があるため、この修正は重要です。
対処方法: v1.31.7の対処方法に準じます。APIクライアント（カスタムコントローラー、オペレーターなど）の安定性を監視し、必要に応じてアップグレードを検討してください。
2. kubelet再起動時のPodボリュームのアンマウント問題修正

変更内容 (Bug or Regression): ユーザーによって参照されているPVCが削除されている場合、kubeletの再起動時に実行中のPodのボリュームがアンマウントされてしまうバグを修正しました。(kubelet再起動時のPodボリューム不適切アンマウント修正 #130684)
プラットフォームへの影響: 高
理由: これもv1.31.7で修正されたものと同様の、kubeletのボリュームアンマウントに関する深刻なバグです。v1.32系列のクラスタの場合も、実行中のPodのボリュームが予期せずアンマウントされる可能性があるため、この修正は必須です。
対処方法: v1.31.7の対処方法に準じます。ステートフルなアプリケーションやPVCを利用している環境では、速やかにこのバージョンにアップグレードすることを強く推奨します。
3. CEL CIDRライブラリの誤判定修正

変更内容 (Bug or Regression): CEL CIDRライブラリにおける問題を修正しました。この問題により、他のCIDR内に含まれるサブネットが、含まれていないものとして誤って拒否されていました。(CEL CIDRライブラリ誤判定修正 #130773)
プラットフォームへの影響: 中
理由: CEL (Common Expression Language) は、ValidatingAdmissionPolicyや一部のCustomResourceDefinition (CRD) validationなどで使用される式言語です。このバグは、CELのCIDR関連の関数が正しく機能しないことを意味します。具体的には、特定のIPアドレスが特定のサブネット内にあるかの検証が誤って拒否される可能性があります。これにより、期待通りのリソース作成や変更ができなかったり、セキュリティポリシーや承認ポリシーが正しく機能しなかったりする可能性があります。
対処方法:
ValidatingAdmissionPolicyやCRDスキーマでCELおよびCIDR関連の検証ルールを使用している場合は、この修正が適用されることで正しく動作するようになることが期待されます。
もし現在もこの問題によりリソース作成や変更がブロックされている状況があれば、アップグレードは必須です。
アップグレード後、関連するAdmission PolicyやCRD検証ルールが期待通りに動作するか確認してください。
総評とプラットフォーム維持のアドバイス
提供されたリリースノートは、Kubernetesのv1.31系とv1.32系の両方におけるパッチリリース（バグ修正やセキュリティ修正が主）であることがわかります。これらのリリースは、既存のクラスタの安定性と信頼性を向上させるために非常に重要です。

特に以下の点に注意してください。

Windowsノードの利用状況の確認:
v1.31.8におけるWindows Proxyのバグ修正は、Windowsノードを使っている環境では最優先で適用すべきです。
ストレージ関連の安定性の向上:
v1.31.7およびv1.32.3の両方で修正されたkubelet再起動時のボリュームアンマウントのバグは、ステートフルワークロードにとって極めて高リスクです。ステートフルなアプリケーションを運用している場合は、速やかにこれらの修正が含まれるバージョンへアップグレードすることを強く推奨します。
API Machineryの安定性:
kube-apiserverのwatchイベントに関するバグ修正も、Kubernetesのコア機能の信頼性に関わるため重要です。カスタムコントローラーなどAPI Watchに依存するコンポーネントが多い環境では安定性向上に寄与します。
ポリシー評価の正確性:
v1.32.3でのCEL CIDRライブラリの修正は、ValidatingAdmissionPolicyなどを利用している環境で、ネットワーク関連のポリシー評価の正確性を確保するために重要です。
一般的な対処方法の原則:

テスト環境での検証: どのようなパッチリリースであっても、本番環境に適用する前に、必ずステージングまたはテスト環境で十分な動作確認を行ってください。特に、影響度が「高」または「中」と評価された項目に該当する機能やアプリケーションのテストを重点的に実施してください。
監視の強化: アップグレード前後は、クラスタの主要コンポーネント（API Server, kubelet, Controller Manager）、ネットワーク、ストレージ、およびアプリケーションのログとメトリクスを通常よりも詳細に監視してください。
計画的なアップグレード: これらのパッチリリースは、通常後方互換性を壊すものではありませんが、重要なバグ修正が含まれているため、運用中のKubernetesバージョンに対応する最新のパッチバージョンへの計画的なアップグレードを継続的に実施することが、プラットフォームの健全性維持に不可欠です。
今回のリリースノートは、特にWindowsノードの利用、ステートフルワークロードの運用、そしてKubernetes APIの安定性という観点から、プラットフォームの維持において重要な意味を持つ変更点が複数含まれています。これらの情報に基づいて、貴社のKubernetesクラスタのアップグレード計画を立て、実行してください。
