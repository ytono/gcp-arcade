...
read -p "Enter a zone 1: " ZONE_1
read -p "Enter a zone 2: " ZONE_2
...
gcloud compute networks subnets create subnet-a --network vpn-network-1 --range 10.1.1.0/24 --region $REGION_1

gcloud compute firewall-rules create network-1-allow-custom --network vpn-network-1 --allow tcp:0-65535,udp:0-65535,icmp --source-ranges 10.0.0.0/8

gcloud compute firewall-rules create network-1-allow-ssh-icmp --network vpn-network-1 --allow tcp:22,icmp
...
gcloud compute networks subnets create subnet-b --network vpn-network-2 --range 192.168.1.0/24 --region $REGION_2

gcloud compute firewall-rules create network-2-allow-custom --network vpn-network-2 --allow tcp:0-65535,udp:0-65535,icmp --source-ranges 192.168.0.0/16

gcloud compute firewall-rules create network-2-allow-ssh-icmp --network vpn-network-2 --allow tcp:22,icmp

gcloud compute instances create server-2 --machine-type=e2-medium --zone $ZONE_2 --subnet subnet-b

## Task 3. Explore the networks and instances
