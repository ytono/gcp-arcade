構成
この構成では、外部からのトラフィックをGLBで受け、内部のGKEサービスへルーティングするためにILB（Gateway API）を経由し、GKEのLBへと転送します。

1. GLB（グローバルロードバランサ）
外部からのHTTPSトラフィックを受け付け、グローバルに分散されたエンドポイントを提供します。
SSL証明書をGLBに設定し、HTTPS通信を終端します。
バックエンドサービスとして、ILB（Gateway API）のIPアドレスを登録します。
2. ILB（内部ロードバランサ、Gateway APIを使用）
VPCネットワーク内で内部トラフィックをルーティングします。
Gateway APIを使用して、GKEのIngressコントローラへトラフィックを転送します。
ルーティングルールを設定し、GLBからのトラフィックを適切なGKEサービスへ転送します。
3. GKE（Google Kubernetes Engine）
コンテナ化されたアプリケーションをデプロイします。
Ingressコントローラを使用して、GKEサービスへのトラフィックをロードバランスします。
Serviceリソースを使用して、Podへのトラフィックを分配します。
確認ポイント
1. GLBの動作確認
外部からGLBのIPアドレスまたはドメイン名にHTTPSリクエストを送信し、正常に応答が返ってくるか確認します。
SSL証明書が正しく設定されているか確認します。
GLBのログを確認し、トラフィックが正常にILBへ転送されているか確認します。
2. ILB（Gateway API）の動作確認
ILBのIPアドレスに内部からリクエストを送信し、GKEサービスへ正常に転送されるか確認します。
Gateway APIのリソース（Gateway、HTTPRouteなど）が正しく設定されているか確認します。
ILBのログを確認し、トラフィックが正常にGKEサービスへ転送されているか確認します。
3. GKEの動作確認
GKEサービスが正常に動作しているか確認します。
Ingressコントローラが正しく設定されているか確認します。
Serviceリソースが正しく設定されているか確認します。
Podのログを確認し、アプリケーションが正常に応答しているか確認します。
4. エンドツーエンドの確認
外部からGLBのIPアドレスまたはドメイン名にHTTPSリクエストを送信し、GKEアプリケーションが正常に応答するかエンドツーエンドで確認します。
各コンポーネントのログを確認し、エラーが発生していないか確認します。
パフォーマンスを確認し、遅延やエラーが発生していないか確認します。
5. セキュリティの確認
GLB、ILB、GKEのファイアウォールルールが適切に設定されているか確認します。
IAMポリシーが適切に設定されているか確認します。
SSL証明書が適切に管理されているか確認します。
6. 障害時の確認
各コンポーネントを意図的に停止させ、フェイルオーバーが正常に動作するか確認します。
GLB、ILB、GKEのログを確認し、障害発生時の挙動を確認します。
モニタリングツールを使用して、障害発生時のアラートが正常に通知されるか確認します。
# Title: March 05, 2025 
